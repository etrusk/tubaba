...
/**
 * Randomize all skills for all characters in debug mode
 * @returns Updated state with randomized skills for each character
 */
export function debugRandomizeAllSkills(state: DebugBattleState): DebugBattleState {
  ...
  const updatedCharacters = state.characters.map(character => {
    // Shuffle all skills and pick 4
    const shuffled = [...allSkills].sort(() => Math.random() - 0.5);
    const selectedSkills = shuffled.slice(0, 4);
    
    // Assign random targeting and priority to each
    const randomizedSkills = selectedSkills.map((skill, index) => ({
      ...skill,
      targeting: TARGETING_MODES[Math.floor(Math.random() * TARGETING_MODES.length)]!,
      priority: index + 1,
    }));

    return {
      ...character,
      skills: randomizedSkills,
    };
  });
